...
sólo debe pausar la operación en curso (instalación, desinstalación, etc)
...
Las tareas básicas en la Administración de Software son :
\begin{itemize}
...
{\bf rpm} es el formato y extensión del nombre de los archivos de paquetes de la distribución Red Hat y muchas otras.
...
El conjunto puede personalizar según el uso que se quiera dar al sistema, como servidor, escritorio, o terminal de trabajo del desarrollador. 
...
Esto puede realizarse directamente con cualquier editor (provisto de que Ud. tiene privilegios de root), o bien a través del 
comando {\tt apt edit-sources}.
...
El comando \texttt{apt-get update} sincroniza el índice de paquetes desde sus
           ...
           archivo Packages.gz, de tal forma que la información acerca de los
...
El paquete nautilus necesita las bibliotecas compartidas mencionadas, así pues APT las descargará del servidor. Si se especifican antes los nombres de esas bibliotecas con el comando apt-get APT no pregunta, si desea continuar o no; supone automáticamente que se desean instalar esos paquetes.
...
e instalarlas. Se debe utilizar la opción upgrade o dist-upgrade de apt-get.

{\bf upgrade}
   
        upgrade se usa para instalar la versión más nueva de todos los paquetes instalados en el sistema provenientes de alguna de las fuentes listadas en /etc/apt/sources.list. Los
           paquetes instalados con una nueva versión disponible se descargan y actualizan, y bajo ninguna circunstancia se desinstalarán paquetes, o se instalarán paquetes nuevos. Las
           nuevas versiones de programas instalados que no se puedan actualizar sin cambiar el estado de instalación de otros paquetes no se instalarán, manteniéndose la versión actual.
           Debe realizar un update antes para que apt-get sepa que hay nuevas versiones disponibles.

{\bf dist-upgrade}
           dist-upgrade, además de realizar las acciones de upgrade, trata inteligentemente los cambios de dependencias debidos a las nuevas versiones de paquetes.  apt-get tiene un
           sistema «inteligente» de resolución de conflictos, y si es necesario tratará de actualizar los paquetes más importantes a costa de los menos importantes. Por ello, puede que
           la orden dist-upgrade elimine algunos paquetes. El archivo /etc/apt/sources.list contiene una lista de sitios desde los que descargar los archivos de paquetes. También puede
           consultar apt\_preferences(5) si quiere invalidar este comportamiento para paquetes individuales.

Es muy importante realizar un \texttt{apt-get update} antes de actualizar el sistema.
...
\subsubsection*{Obtener Información de todos los paquetes instalados}

El comando {\tt apt list --installed} nos permite obtener un listado de todos los paquetes instalados en el sistema.
